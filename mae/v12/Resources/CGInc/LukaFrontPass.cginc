...
_RBGrainPower, _RBGrainSpeed, _RBGrainBlack, _RBInverse, _RBBW, _RBDarkness, _RBExposure, _RBRadius, _RBDistortionX, _RBDistortionY, _RBDistortionXSpeed, _RBDistortionYSpeed, _RBColoringToggle, _RBDistortionToggle;
sampler2D _RBDistortionMap;
float4 _RBDistortionMap_ST;
float4 _RBEDBackColor, _RBEDColor, _RBGrainColor, _RBColorTest, _RBColoring;

//recolor 
float _ToggleRecolor, _RecolorBright, _RecolorSat, _RecolorHue, _RecolorSpeed, _ToggleRecolorAnimate;
float _HueMethod, _RecolorMethod;
...
//edge width stuff
float _EDWidth, _EPWidth, _ROWidth;
...
//blocky glitch
float _ToggleBlockyGlitch, _AllowBGX, _AllowBGY, _BDepthX, _BDepthY, _BlockyGlitchStrength, _BGRandomnessInc, _BlockyGlitchSpeed, _BGMethod;
sampler2D _BlockGlitchMap;
float4 _BlockGlitchMap_ST;
float _ArtPower, _ArtSize, _ArtDis;
...
//vignette
float _ToggleVignette, _VigX, _VigColPow, _VigMode, _VigSharpness, _VigFlicker;
float4 _VigCol;
float _VigToggleHSVRainbow, _VigToggleHSVRainbowX, _VigToggleHSVRainbowY, _VigHSVRainbowHue, _VigHSVRainbowSat, _VigHSVRainbowLight, _VigHSVRainbowTime;
...
float _HSVToggleDistortion, _HSVDistortionX, _HSVDistortionY, _HSVDistortionXSpeed, _HSVDistortionYSpeed;
sampler2D _HSVDistortionMap;
float4 _HSVDistortionMap_ST;
...
//gravity
float _ToggleGravity, _GravityMethod, _GravityRadius;
...
//new blur
float _ToggleNBlur, _NBlurShape, _NBlurItterations, _NBlurPower, _NBlurSpeed, _NBlurRotate, _NBlurRotateSpeed, _NBlurX, _NBlurY, _NBlurOpacity, _NBlurGlow, _NBlurMethod;
float4 _NBlurColor;
...
float _GDDistortionX, _GDDistortionY, _GDDistortionXSpeed, _GDDistortionYSpeed;
...
//swirl
float _ToggleSwirl, _SwirlPower, _SwirlCenterX, _SwirlCenterY, _SwirlRadius, _SwirlSpeed;
...
//motion blur
float _MotionBlurPower, _MotionBlurVR, _MBToggleED, _MBEDTolerance, _MBEDTrans,
_MBEDWidth, _MBDistortionToggle, _MBDistortionX, _MBDistortionY, _MBDistortionXSpeed,
_MBDistortionYSpeed, _MBEDLighting, _MBToggleEDOnly, _MBMethod, _MBX, _MBY, _MBDualPass,
_MBGreen, _MBBlue;
float4 _MBEDColor;
sampler2D _MBDistortionMap;
float4 _MBDistortionMap_ST;

//last frame data
sampler2D _LukaFrameData;
float4 _LukaFrameData_ST;
...
half4 frag(v2f i, VertexOutput vo, float facing : VFACE) : SV_Target: COLOR
{
	...
	else
	{
		...
		//Applying additional rendering options
		if (isInVR()) {
			returnOpacity *= _VRAdjust;
		}
		if (isInMirror()) {
		...
		UNITY_BRANCH
		if (_PlatformRendering == 1) {
			if (!isInVR()) {
				return bgcolor;
			}
		}
		else if (_PlatformRendering == 2) {
			if (isInVR()) {
				return bgcolor;
			}
		}
		...
		UNITY_BRANCH
		if (_MirrorRendering == 1) {
			if (!isInMirror()) {
				return bgcolor;
			}
		}
		else if (_MirrorRendering == 2) {
			if (isInMirror()) {
				return bgcolor;
			}
		}
		...
		if (_ToggleBlockyGlitch) {
			...
			if (_BGMethod == 0) {
				...
				float2 blockSize = float2(_ArtSize, _ArtSize);
				...
				if (tex2D(_BlockGlitchMap, uv_noise).r * _ArtPower < block_thresh) {
					float testMe = randomNegativeChance(0.5, 0.5);
					if (testMe < 0) {
						sceneUVs.y += _ArtDis;
					}
					else {
		...
		if (_ToggleGravity) {
			...
			UNITY_BRANCH
				if (_GravityMethod == 0) {
					float newDistance = smoothstep(.5, 1., _GravityRadius - dist); //1 is radius centre
					sceneUVs = lerp(0, sceneUVs.xy, 1 - newDistance);
				}
				else {
					float newDistance = smoothstep(.5, 1., 2  - _GravityRadius - dist); //1 is radius centre
					sceneUVs = lerp(0, sceneUVs.xy, newDistance);
				}
		}
		...
		//Effect: GIF Distortion
		if (_ToggleGifOverlay && _GifMethod == 1) {
			float2 transformDistortion = float2(_GDDistortionX, _GDDistortionY);
			...
			float2 distortionUVs = TRANSFORM_TEX(_twoUVMath, _OverlaySpritesheet);

			//Distortion speed option
			UNITY_BRANCH
			if (_GDDistortionXSpeed > 0 || _GDDistortionYSpeed > 0) {
				float2 distortionSpeed = (float2(_Time.y * _GDDistortionXSpeed, _Time.y * _GDDistortionYSpeed));
				distortionUVs = (distortionSpeed * transformDistortion + sceneUVs);
			}


			float4 transformMap = tex2D(_OverlaySpritesheet, TRANSFORM_TEX(distortionUVs.xy, _OverlaySpritesheet));

			//Distortion transparency option
			sceneUVs.xy += (((transformMap.rgb*transformDistortion)));
		}
		...
		if (_ToggleSwirl) {
			...
			if (calcSwirlDistance < _SwirlRadius)
			{
				...
				_SwirlPower += sin(_Time.y*_SwirlSpeed);
		...
		if (_NBlurMethod == 1) {
			...
			UNITY_BRANCH
				if (_ToggleRadialBlur)
				{
					...
					//distance to center
					half dist = sqrt(dir.x * dir.x + dir.y * dir.y);  
					float newDistance = smoothstep(.5, 1., _RBRadius - dist);
					UNITY_BRANCH
						if (_RBInverse == 1) {
					...
					[unroll(64)]for (int copyLoop = 0; copyLoop < _RBItterations; copyLoop++) { 
						...
						UNITY_BRANCH
							if (_RBMode == 2) {
								...
								float calculatedStrength = copyLoop * 0.07;
								float4 testColor = tex2D(_LukaFrontPass, lerp(lerp(sceneUVs, thisUV, calculatedStrength * _RadialBlurDistance), sceneUVs, 1 - newDistance));
								if (testColor.g > _RBColorTest.r && testColor.r > _RBColorTest.b && testColor.r > _RBColorTest.g) {
									radialUVs = lerp(lerp(sceneUVs, thisUV, calculatedStrength * _RadialBlurDistance), sceneUVs, 1 - newDistance);
								}
							}
							else if (_RBMode == 5) {
								float calculatedStrength = copyLoop * 0.07;
								float4 testColor = tex2D(_LukaFrontPass, lerp(sceneUVs, thisUV, calculatedStrength * _RadialBlurDistance));
								if (testColor.g > _RBColorTest.r && testColor.r > _RBColorTest.b && testColor.r > _RBColorTest.g) {
									radialUVs = lerp(sceneUVs, thisUV, calculatedStrength * _RadialBlurDistance);
								}
							}
						...
						//Distortion option
						UNITY_BRANCH
						if (_RBDistortionToggle) {
							float2 transformDistortion = float2(_RBDistortionX, _RBDistortionY);
							float2 distortionUVs = sceneUVs;

							UNITY_BRANCH
							if (_RBDistortionXSpeed > 0 || _RBDistortionYSpeed > 0) {
								float2 distortionSpeed = (float2(_Time.y * _RBDistortionXSpeed, _Time.y * _RBDistortionYSpeed));
								distortionUVs = (distortionSpeed * transformDistortion + sceneUVs);
							}

							float4 transformMap = tex2D(_RBDistortionMap, TRANSFORM_TEX(distortionUVs.xy, _RBDistortionMap));
							radialUVs.xy += (((((transformMap.rgb*transformDistortion))) * copyLoop * _RBEmpower * (1 - newDistance)));
						}
						...
						//Color option
						UNITY_BRANCH
							if (_RBColoringToggle) {
								float colorEmpower = _RBEmpower * 2.5;
								float4 coloredSample = sampledTexture;
								UNITY_BRANCH
									if (!(_RBColoring.r + _RBColoring.g + _RBColoring.b == 1)) {
										coloredSample *= (_RBColoring*(colorEmpower*copyLoop));
									}
								UNITY_BRANCH
									if (_RBBW > 0) {
										coloredSample.rgb = lerp(coloredSample.rgb, dot(coloredSample.rgb, float3(0.3, 0.59, 0.11)), _RBBW);
									}
								UNITY_BRANCH
									if (_RBDarkness > 0) {
										coloredSample.rgb = coloredSample.rgb*(1.0 - clamp(0, 1, (_RBDarkness*copyLoop*colorEmpower)*5));
									}
								UNITY_BRANCH
									if (_RBExposure > 0) {
										coloredSample = coloredSample * (_RBExposure*(1+ colorEmpower)*copyLoop);
									}
								sampledTexture = lerp(sampledTexture, coloredSample, 1 - newDistance);
								// draws a ring sampledTexture = lerp(sampledTexture, coloredSample, (1 - newDistance)*1.3);
							}
						...
						UNITY_BRANCH
							if (_RBGrainPower > 0) {
								float storePower = _RBGrainPower;
								_RBGrainPower += (1 - newDistance) * (copyLoop * (_RBEmpower * 20) * (lerp(_RadialBlurDistance, 0, newDistance) * 5));
								...
								float3 staticTexture = lerp(sampledTexture.rgb, lerp(normalStatic, blackStatic, _RBGrainBlack), (1 - newDistance)*0.8);
								sampledTexture.rgb = lerp(sampledTexture.rbg, staticTexture.rgb, 1);
							}
		...
		else if (_NBlurMethod == 3) {

			if (isInVR()) {
				_MotionBlurPower *= _MotionBlurVR;
			}

			UNITY_BRANCH
			if (_MotionBlurPower > 0) {
				UNITY_BRANCH
					if (_MBMethod == 0) {
						[unroll(16)]for (int i = 0; _MotionBlurPower > i; i++) {
							float2 mbUVs = sceneUVs;
							mbUVs.xy += float2(_MBX, _MBY);
							//Distortion option
							UNITY_BRANCH
								if (_MBDistortionToggle) {
									float2 transformDistortion = float2(_MBDistortionX, _MBDistortionY);
									float2 distortionUVs = sceneUVs;

									UNITY_BRANCH
										if (_MBDistortionXSpeed > 0 || _MBDistortionYSpeed > 0) {
											float2 distortionSpeed = (float2(_Time.y * _MBDistortionXSpeed, _Time.y * _MBDistortionYSpeed));
											distortionUVs = (distortionSpeed * transformDistortion + sceneUVs);
										}

									float4 transformMap = tex2D(_MBDistortionMap, TRANSFORM_TEX(distortionUVs.xy, _MBDistortionMap));
									mbUVs.xy += (((((transformMap.rgb*transformDistortion))) * i * 0.02));
								}

							float4 pastFrame = float4(tex2D(_LukaFrameData, mbUVs));

							//Outline option
							UNITY_BRANCH
								if (_MBToggleED) {
									float3 edgeColor = createEdge(mbUVs, _MBEDWidth, _MBEDTolerance, 0, 0, _MBEDColor, _LukaFrameData);
									pastFrame.rgb = edgeColor;
								}


							grabToChange += pastFrame;
						}
						UNITY_BRANCH
							if (_MBToggleED && _MBToggleEDOnly) _MotionBlurPower *= _MBEDLighting;
						grabToChange /= _MotionBlurPower + 1;
						...
						[unroll(16)]for (int i = 0; _MotionBlurPower > i; i++) {
							float2 addUVs = float2(_MBX, _MBY);
							float2 redUVs = addUVs;
							float2 greenUVs = addUVs + float2(_MBGreen, 0);
							float2 blueUVs = addUVs + float2(_MBBlue, 0);
							float4 pastFrame = tex2D(_LukaFrameData, sceneUVs);
							pastFrame.r = tex2D(_LukaFrameData, sceneUVs + redUVs).r;
							pastFrame.g = tex2D(_LukaFrameData, sceneUVs + greenUVs).g;
							pastFrame.b = tex2D(_LukaFrameData, sceneUVs + blueUVs).b;
							grabToChange += pastFrame;
						}
						grabToChange /= _MotionBlurPower + 1;
					}
		...
		if (_ToggleSmear) {
			...
			[unroll(16)]for (int i = 0; i < _CSCopies; i++) { //kms for loops 
		...
		if (_ToggleED) {
			...
			//applying ramp ~ unused cos i need the samplers
		...
		UNITY_BRANCH
		if (_ToggleHSVRainbow > 0 && _HueMethod == 1) {
			...
			UNITY_BRANCH 
				if (_RecolorMethod == 1) {
					...
					float ds = distance(wpos, _WorldSpaceCameraPos);
					ds *= 0.25;
					methodMod = (clamp(-20, 20, ds / _RecolorHue)) / 20;
				}
				else if (_RecolorMethod == 2) {
					//VR Support
					#if UNITY_SINGLE_PASS_STEREO
					half2 dir;
					if (sceneUVs.x < 0.5) {
					...
					//distance to center
					half dist = sqrt(dir.x * dir.x + dir.y * dir.y);
					addMod = smoothstep(.5, 1., 1.15 - dist);
				}
			UNITY_BRANCH
				if (_ToggleRecolorAnimate) {
					_RecolorHue *= (_SinTime.x)*methodMod;
				}
				else {
					_RecolorHue *= methodMod;
				}
			_RecolorHue += addMod;
			...
			grabToChange.rgb = lerp(grabToChange.rgb, lerp(staticHue, animatedHue, _ToggleRecolorAnimate), _ToggleHSVRainbow);
		}
		...
		if (_ToggleVHS) {
			...
			fixed window = 1 / (1 + 20 * (uv.y - fmod(_Time.y*_VHSYDisplacement / 4, 1)) * (uv.y - fmod(_Time.y*_VHSYDisplacement / 4, 1)));
			fixed vShift = 0.4 *(sin(_Time.y*_VHSYDisplacement) + (sin(_Time.y*_VHSYDisplacement) * cos(_Time.y*_VHSYDisplacement)));
			uv.y = fmod(uv.y + vShift, 1);
			//30 = striples count
			grabToChange = mul(grabToChange, (_darkness + fmod(uv.y * _VHSXDisplacement + _Time.y*_VHSYDisplacement, 1)) / (_darkness + 1));
		...
		if (_ToggleVignette > 0) {
			//rainbow
			UNITY_BRANCH
			if (_VigToggleHSVRainbow > 0) {
				float hueMod = _Time.y*_VigHSVRainbowTime; //set up time
				hueMod += (sceneUVs.y*_VigToggleHSVRainbowY); //vertical multiplier
				hueMod += (sceneUVs.x*_VigToggleHSVRainbowX); //horizontal multiplier
				hueMod *= _VigHSVRainbowHue;
				float3 hsvColor = smoothHSV2RGB(float3(hueMod, _VigHSVRainbowSat, _VigHSVRainbowLight));
				_VigCol.rgb = lerp(_VigCol.rgb, _VigCol.rgb*hsvColor, _VigToggleHSVRainbow);
			}

			//setting up vignette
			float2 vigUVs = sceneUVs * (1. - sceneUVs);
			float vignette = (pow(vigUVs.x*vigUVs.y*_VigSharpness, _VigX));
			UNITY_BRANCH
			if (_VigFlicker > 0) {
				float vignetteFlicker = valueRemap(sin(_Time.y * _VigFlicker), 0, 100, 0.4, 4.8);// clamp(0, 0.3, (abs(sin(_Time.x * 10))));
				vignette *= vignetteFlicker;
			}
			...
			if (_ToggleVignette > 0) grabToChange = lerp(grabToChange, hardVig, _ToggleVignette);
		}

		//Effect: Rainbow
		UNITY_BRANCH
		if (_ToggleHSVRainbow > 0 && _HueMethod == 0) {
			float methodMod = 1;
			float addMod = 0;
			UNITY_BRANCH
				if (_RecolorMethod == 1) {
			...
			float2 distortRainbowUV = float2(0, 0);
			UNITY_BRANCH
				if (_HSVToggleDistortion) {
					float2 transformDistortion = float2(_HSVDistortionX, _HSVDistortionY);
					float2 distortionUVs = sceneUVs;
					if (_HSVDistortionXSpeed > 0 || _HSVDistortionYSpeed > 0) {
						float2 distortionSpeed = (float2(_Time.y * _HSVDistortionXSpeed, _Time.y * _HSVDistortionYSpeed));
						distortionUVs = (distortionSpeed * transformDistortion + sceneUVs);
					}
					float4 transformMap = tex2D(_HSVDistortionMap, TRANSFORM_TEX(distortionUVs.xy, _HSVDistortionMap));
					distortRainbowUV = sceneUVs + (((transformMap.rgb*transformDistortion)));
				}
			float hueMod = methodMod*_Time.y*_HSVRainbowTime; //set up time
			hueMod += (sceneUVs.y*_ToggleHSVRainbowY); //vertical multiplier
			hueMod += (sceneUVs.x*_ToggleHSVRainbowX); //horizontal multiplier
			hueMod += distortRainbowUV;
